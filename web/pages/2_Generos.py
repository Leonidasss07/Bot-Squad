...
/* Estrella favorito */
div[data-testid="stMain"] div.stButton > button[title="Añadir a favoritos"] {
...
div[data-testid="stMain"] div.stButton > button[title="Añadir a favoritos"]:hover {
    background: transparent !important;
    transform: scale(1.2);
}
</style>
...
# Solo muestra géneros cuyos CSV existen
...
if not TAGS_DISPONIBLES:
    ...
    # Intenta cruzar con canciones_populares.csv para obtener reproducciones reales
    ...
    # Si no hay datos numéricos, usa posición
    ...
    with col_g:
        if valor_col and len(tag_df) > 0:
            ...
            # Divide entre millones para que el eje no use notación científica
            if not usar_ranking:
